...
def duffing_param(U, constant):
    ...
    omegan = np.sqrt(abs(k1/m)) # natural frequency (no damping)

    zeta = d/(2*omegan*m)
    omegad = omegan*np.sqrt(1-zeta**2) # natural frequency including damping
...
def duffing_oscillator_friction_nondim(x, constant):
    ...
    xprime = (-dbar*x1 - k1bar*x2 - k3bar*x2**3 - Fbar*np.sign(x1), x1)
...
def fixed_points_nondim(U, constant):
    ...
    if U <= U_D:
        ...
        Xf2 = (0, np.sqrt(-k1bar/k3bar))
        Xf3 = (0, -1.*np.sqrt(-k1bar/k3bar))
...
def speed_critical(constant):
    ...
    U_D = np.sqrt(kalfa1/(2.*pi*rho*b**2*(a + 0.5)))
