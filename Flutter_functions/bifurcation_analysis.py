...
def tau_hopf(eig_vals, eps = 1e-13):
    ...
    i_cmp = np.where(np.absolute(eig_vals.imag) > eps)
    ...
    if vals_complex.size == 0:
        ...
        for i, val in enumerate(vals_complex):
            i_cc = np.where(
                (np.absolute(val.real - vals_complex.real) < eps) & (np.absolute(val.imag + vals_complex.imag) < eps))[0][0]
            tau_h *= sc.sqrt(val.real + vals_complex[i_cc].real)

    return np.real(tau_h)
...
def tau_pitchfork(eig_vals):
    ...
    # =================================================
    i_real = np.where(np.absolute(eig_vals.imag) < eps)
...
#
#     # return np.absolute(val_cmp_min.real)
#     # ----------------------------------------------------------------------------------------------------------------
#
#     eps = 1e-15
#
...
#
#     i_cmp_min = np.absolute(vals_cmp.real).argmin()
...
#
#     # return np.absolute(val_real_min)
#     # ----------------------------------------------------------------------------------------------------------------
#
#     eps = 1e-15
#
...
#     vals_real = eig_vals[i_real].real
#
#     i_real_min = np.absolute(vals_real).argmin()
...
def R1_hopf_modified(Q, dQdU):
    ...
    # determine R1:
    i_cmp = np.where(np.absolute(vals.imag) > eps)[0]
    vals_cmp = vals[i_cmp]

    i_cmp_min = i_cmp[np.absolute(vals_cmp.real).argmin()]
...
def R1_pitchfork_modified(Q, dQdU):
    ...
    # Determine R1:
    i_real = np.where(np.absolute(vals.imag) < eps)[0]
    vals_real = vals[i_real].real

    i_real_min = i_real[np.absolute(vals_real).argmin()]
...
def locatate_bifurcation(func, U_min, U_max, type):
    ...
    eval_bf = evals[np.absolute(evals.real).argmin()]

    if type == 'hopf':
        omega_bf = np.absolute(eval_bf)
        zeta_bf = -eval_bf.real/np.absolute(eval_bf)
    elif type == 'pitchfork':
        omega_bf = 0
        zeta_bf = 0
