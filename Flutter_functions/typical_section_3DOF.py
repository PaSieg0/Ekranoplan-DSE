...
def matrix_mass_str(constant):
    ...
    A = np.array([[m, S, Sbt], [S, Ith, Ith_bt], [Sbt, Ith_bt, Ibt]])
...
def vector_init_condt(x0, constant, T):
    ...
    g = b*(h0 + b*(0.5 - a)*th0 + b*T11*bt0/(2.*pi))*np.array([2.*pi, -2.*pi*b*(a + 0.5), b*T12])
...
def typical_section_3DOF_nonlin(x, U, constant, T, coefW):
    ...
    qn_plunge = np.zeros((12,))
    qn_plunge[:3] = -M_inv.dot([Kh3, 0., 0.])

    qn_pitch = np.zeros((12,))
    qn_pitch[:3] = -M_inv.dot([0., Kth3, 0.])

    qn_control = np.zeros((12,))
...
def fixed_points_3DOF_nonlin_th(U, constant, T, coefW):
    ...
    # th value:
    thF2 = np.sqrt((qn.dot(Qlinv.dot(ql)) - qnn)/Kth3)
    thF3 = (-1.)*np.sqrt((qn.dot(Qlinv.dot(ql)) - qnn)/Kth3)
    ...
    xF1, xF2, xF3 = np.zeros((12,)), np.zeros((12,)), np.zeros((12,))
...
def typical_section_3DOF_eqlin(x, A, U, constant, T, coefW):
    ...
    # Linear part:
    # ============
    Q_lin = typical_section_3DOF_lin(np.identity(12), U, constant, T, coefW)
...
def jacobian_nonlin(x0, U, constant, T, coefW):
    ...
    # Linear part:
    jac_lin = typical_section_3DOF_lin(np.identity(12), U, constant, T, coefW)
...
def stability_LCO_eqlin(A_up, A_down, U, constant, T, coefW):
    Q_up = typical_section_3DOF_eqlin(np.identity(12), A_up, U, constant, T, coefW)
    evals_up = ln.eigvals(Q_up)

    Q_down = typical_section_3DOF_eqlin(np.identity(12), A_down, U, constant, T, coefW)
...
def tvla(dt, x0, U, constant, T, coefW, delta=1e-3):
    ...
    while True:
        dx = np.zeros((12,), dtype='complex')
        for i in range(12):
            dx += -1*V[:, i]*(1/L[i])*(1 - np.exp(L[i]*dt))*b[i]
        ...
        dx_bar = np.zeros((12,), dtype='complex')
        for i in range(12):
            dx_bar += -1*V_bar[:, i]*(1/L_bar[i])*(1 - np.exp(-1.*L_bar[i]*dt))*b_bar[i]

        error = dx.real + dx_bar.real
        eps = np.sqrt(error.dot(error))
