...
def get_amp_freq(t, x):
    ...
    t_max = np.zeros(idx_max.shape)
    amp_max = np.zeros(idx_max.shape)

    t_min = np.zeros(idx_min.shape)
    amp_min = np.zeros(idx_min.shape)
    ...
    for idx in idx_max:
        if idx != 0 and idx < len(x) - 1:
            ...
            A = np.array([[1, t[idx - 1], t[idx - 1]**2],
                          [1, t[idx], t[idx]**2],
    ...
    t_omega_max = t_max[:-1] + 0.5*np.diff(t_max)
    omega_max = 2*np.pi/np.diff(t_max)

    t_omega_min = t_min[:-1] + 0.5*np.diff(t_min)
    omega_min = 2*np.pi/np.diff(t_min)

    # Postprocess:
    # ============
    # t_amp = np.append(t_max, t_min)
    # amp = np.append(amp_max, np.absolute(amp_min))
    # id_sort = np.argsort(t_amp)
    # t_amp = t_amp[id_sort]
    # amp = amp[id_sort]

    t_amp = t_max
    amp = np.zeros(t_max.shape)
    mean = np.zeros(t_max.shape)
    ...
    # t_amp = np.zeros(t_max.shape)
    # amp = np.zeros(t_max.shape)
    # mean = np.zeros(t_max.shape)
    #
    # for i_max, t_max_i in enumerate(t_max):
    #     i_min = np.absolute(t_min - t_max_i).argmin()
    #     amp[i_max] = (amp_max[i_max] - amp_min[i_min])/2.
    #     mean[i_max] = (amp_max[i_max] + amp_min[i_min])/2.
    #     t_amp[i_max] = (t_max_i + t_min[i_min])/2.

    t_omega = np.append(t_omega_max, t_omega_min)
    omega = np.append(omega_max, omega_min)
    id_sort = np.argsort(t_omega)
...
def resample(t, x, dt_resample):
    ...
    t_res = np.arange(t_min, t_max, dt_resample)
...
def cluster_amplitudes(amp, eps=0.5):
    ...
    amp_class_mean = np.zeros((n_class,))
    for j, lbl_j in enumerate(lbls_unique):
        if lbl_j != -1:
            mask_class_member = (lbls == lbl_j)
            amp_class_mean[j] = np.mean(amp[mask_class_member])
...
def cluster_stable_segments(stable, eps=0.1):
    ...
    # =================================================================================
    m_stbl = np.array((stable==True))

    # Create index column:
    # =====================
    idx = np.array(range(len(stable)))
    ...
    # ==============================================================================
    idx_stbl_rshp = np.float64(idx_stbl.reshape((-1,1)))
    idx_ustbl_rshp = np.float64(idx_ustbl.reshape((-1, 1)))
